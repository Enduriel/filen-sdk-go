...
func newV2(email, password string, info *client.V3AuthInfoResponse, unauthorizedClient *client.UnauthorizedClient) (*Filen, error) {
	...
	response, err := unauthorizedClient.PostV3Login(context.Background(), email, derivedPass)
	if err != nil {
	...
	mkResponse, err := c.PostV3UserMasterKeys(context.Background(), encryptedMasterKey)
	if err != nil {
...
func newV3(email, password string, info *client.V3AuthInfoResponse, unauthorizedClient *client.UnauthorizedClient) (*Filen, error) {
	...
	mkResponse, err := c.PostV3UserMasterKeys(context.Background(), encryptedKEK)
	if err != nil {
	...
	// dek decryption
	encryptedDEK, err := c.GetV3UserDEK(context.Background())
	if err != nil {
...
func New(email, password string) (*Filen, error) {
	...
	// fetch salt
	authInfo, err := unauthorizedClient.PostV3AuthInfo(context.Background(), email)
	if err != nil {
		return nil, err
	}
